...
export default async function ProgressBar({ participation }: { participation: Participation }) {
  ...
  const progressPercent = Math.ceil(Math.min((progress / courseLength) * 100, 100))

  return (
    <Link
      href={`/dashboard/participation/${participation.id}`}
      className="relative w-[200px] bg-blue-500 flex flex-col hover:bg-blue-600 rounded overflow-hidden transition mt-8"
    >
      <div className="flex flex-row items-center justify-between pl-10">
